...
OUT_PNG = paths.RESULTS / "poc15_wiggle.png"
OUT_JSON = paths.RESULTS / "poc15_wiggle.json"
...
def ladder(shape: str) -> list[dict]:
    ...
    # Imported HERE, not at module top. This module's product half is
    # `arclength` and `wander`; the rest is the POC 15 experiment. Importing the
    # stimulus generators at the top made the server's dependency closure drag
    # a dinosaur-damage builder in behind one function, which is how 71 flat
    # files stopped being separable into product and lab notebook.
    import sys, pathlib
    sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent / "experiments"))
    from poc4_metric_fix import smooth_noise            # noqa: F401
    from poc13_dino_stimuli import (ALPHAS, deepest_fjord,   # noqa: F401
                                    fill_span, match_noise)
